...
def settings_menu(revision: int, settings: dict | None = None):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"⏱ Scan: {_value(settings, 'scan_interval_sec', '3')}s", callback_data=f"menu:scan:{r}"), InlineKeyboardButton(f"🧵 Concurrency: {_value(settings, 'scanner_concurrency', '5')}", callback_data=f"menu:concurrency:{r}")],
